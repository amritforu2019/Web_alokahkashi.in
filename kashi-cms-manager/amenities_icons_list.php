...
if (isset($_GET['del'])) {
 try {
  $qry = $PDO_LINK->prepare("SELECT project_id, image_url,icon_name image FROM tbl_amenities_icons WHERE id = :id");
  ...
  if ($row) {
   ...
   $stmt = $PDO_LINK->prepare("DELETE FROM tbl_amenities_icons WHERE id = :id");
   $stmt->execute(['id' => $_GET['del']]);

   $_SESSION['msg'] = array('success', 'Deleted Successfully');
   header("location:amenities_icons_list?project_id=" . $row['project_id']);
   exit;
  } else {
   $_SESSION['msg'] = array('error', 'Record not found');
   header("location:amenities_icons_list?project_id=" . $_GET['project_id']);
   exit;
  }
 } catch (Exception $e) {
  ...
  <div class="main-content">
    <div class="main-content-inner">
      ...
      <div class="page-content">
        ...
        <div class="row">
          <div class="col-xs-12 text-right"> <a href="amenities_icons?project_id=<?php echo $_GET['project_id'];?>"
                                                class="btn
          btn-primary"  > <i
        ...
        <div class="row">
              ...
              <div class="pull-right tableTools-container"> </div>
                ...
                <thead>
                  <tr>
                      ...
                      <th><i class="menu-icon"></i>Icon Name</th>
                      <th><i class="menu-icon fa fa-image"></i> Icons Image</th>
                      <th>Action</th>
                ...
                <?php
				$i=1;
                $qry = $PDO_LINK->prepare("SELECT * FROM tbl_amenities_icons WHERE project_id = :project_id ORDER BY project_id DESC");
          ...
          src="' . $row['image_url'] . $row['image'] . '"  style="border-radius: 0%;max-width: 100px;"></a>';
                  ...
                  <td>
                   <a class="red" onClick="return del(<?php echo $row['id']; ?>, 'amenities_icons_list')"> <i class="ace-icon
