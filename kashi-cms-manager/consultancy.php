...
if(isset($_GET['del']))
{
$qry=mysqli_query($DB_LINK,"select * from tbl_cons_post where id=".$_GET['del']) or die(mysqli_error());    
	$row=mysqli_fetch_array($qry);	
	unlink('assets/upload/const/'.$row['file_data']);

    mysqli_query($DB_LINK,"delete from tbl_cons_post where id=".$_GET['del']) or die(mysqli_error());
...
{
    mysqli_query($DB_LINK,"update tbl_cons_post set status=0 where id=".$_GET['ban']) or die(mysqli_error());
...
{
    mysqli_query($DB_LINK,"update tbl_cons_post set status=1 where id=".$_GET['unban']) or die(mysqli_error());
...
<meta charset="utf-8" />
<title>Consultancy List :: <?php echo $SITE_NAME;?></title>
...
<?php include('include/header.php');?>
  ...
  <div  class="main-content">
          ...
          <div class="col-xs-12">
            <div class="clearfix">
              <div class="pull-right tableTools-container"> </div>
                ...
                <?php
				$i=1; 
				$qry=mysqli_query($DB_LINK,"select * from tbl_cons_post  order by id desc");
				foreach($qry as $row)
				{  
                    ...
                    <tr class="table-activity">
                      ...
                      <div class="tools action-buttons"><?php /*?> <a href="job_edit?edit=<?php echo $row['id'];?>" title="Edit" class="blue"><i class="ace-icon fa fa-pencil bigger-125"></i></a>
                       ...
                       <?php if($row['status']==1){ ?>
                        ...
                        <?php }?><?php */?>
                       <a class="red"onClick="return del(<?php echo $row['id'];?>, 'consultancy')"> <i class="ace-icon fa fa-times bigger-125" title="Delete"></i> </a> 
                       </div>
                      </td>
   ...
   <td><?php echo  date('d/m/y', strtotime($row['subscription_date']));?></td>
                        ...
                        <td> <a   href="" title="<?php echo $row['query'];?>" >View</a>
                        <br>
                        <br>
                        <a  target="_blank"  href="assets/upload/const/<?php echo $row['file_data'];?>" >Docs</a>
                        </td> 
